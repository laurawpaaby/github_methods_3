...
*cue: is numeric and stay in that way.*
*task: is a character and should be a factor*
```{r}
data$task <- as.factor(data$task)
```

*target_type:was a character, should be a factor - anova and other functions don't get characters*
```{r}
...
trial_model<- glm(correct ~ target.contrast*subject, data = df_m, family = "binomial")
...
plot2
```
*for the training I made both a no pool and complete pool after the best of my knowledge, but even though I make the two different models and use the fitted values for each of them, the plots are still identical ....*
...
df_1 <- data %>% 
  filter(trial.type == "experiment" & subject == "001") 
df_2 <- data %>% 
  filter(trial.type == "experiment" & subject == "002") 
df_3 <- data %>% 
  filter(trial.type == "experiment" & subject == "003") 
df_4 <- data %>%  
  filter(trial.type == "experiment" & subject == "004")
...
```
*This gives the message:*vboundary (singular) fit: see ?isSingular *meaning that this model now results in a singular fit due to random intercepts added.*
```{r}
