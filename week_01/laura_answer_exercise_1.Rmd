...
```
*beta hat is a vector representing the  slope, -5.34, and the intercept. The slope indicates that for each time the weight goes up by 1 lb/1000, the fuel use decreases by 5.34 - intuitively this makes no sense though ... The intercept is 37.23 which here does not make sense to interpret, since it is the fuel use when a car has the weight of 0*
...
Y
```
This gives the actual values of y. 
...
ggplot(mtcars, aes(wt)) +                    # basic graphical object
  geom_point(aes(y=est_Y), colour="red") +
  geom_line(aes(y=est_Y), colour="pink") +
  geom_point(aes(y=Y), colour="blue") +
  ylab("Y") + xlab('Weight (lb/1000)') + 
  ggtitle("Illustration of Y, Estimated Y and Errors") + 
  geom_linerange(aes(residuals, ymin = Y, ymax = est_Y, x = wt)) 
...
b_hat
```
*This is now the estimated beta values: the intercept B0 is here 49,93 and the slope is -13.38. This is now our acquired beta hat values, and they seem quite far from the lm generated - but lets compare ...*
...
ggplot(mtcars, aes(wt)) +                    # basic graphical object
  ...
  geom_point(aes(y= y_hat_new), colour="darkgreen") +
...
```
The green line is the quadratic fit, based on the OLS estimator, the blue is the actual data points, and the red is the datapoints generated from the linear model. 
...
*the blue and green line seem to fit there datapoints equally good ... *
...
y_model3 <- b_hat3[1]*X_df3$`(Intercept)` + b_hat3[2]*X_df3$wt + b_hat3[3]*X_df3$wt_squared + b_hat3[4]*X_df3$wt_3 #calculating y values based on the OLS estimated bhat
...
mean(mtcars$mpg)
```
It is identical to the mean of the y-value, since the modal is made 
...
The probability of the Pontiac Firebird has automatic transmission is 3,1% given its weight.
...
```
According to the AIC fucntion, the non-squared logistic model provides the better fit. 
