...
plot1 <- ggplot(df_m, aes(target.contrast, fitted_y, color = correct)) + 
    ...
    ggtitle("Subplot for Each Subject Fitted")  

plot2 <- ggplot(df_m, aes(target.contrast, predicted, color = correct)) + 
    geom_point()+
    facet_wrap(~subject) +
    theme_minimal()+
    xlab("Target Contrast")+
    ylab('The Fitted Values') +
    ggtitle("Subplot for Each Subject Predict")  
