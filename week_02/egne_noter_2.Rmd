...
`lmer` is used for multilevel modeling
```{r}
#loading libraries:
library(lme4)
```
...
sigma(model4)
```
*Looking at both the standard deviation and the information criterion, we find that the model4 is the best fittet model to the data, since it has the smallest value both places. This makes intuitively good sense, since it is the only model that both have the subject and scenario as baseline, , which means *


```{r}
#vi the most variance explained by the two level models
pacman::p_load(MuMIn)

r.squaredGLMM(model3)
r.squaredGLMM(model4)
```
*Looking at the R2c (which takes both random and fixed effects into account) model 4 explains the most variance.*


    
2) Why is our single-level model bad?
*the single level model is bad, since it violates the most important assumptions of independency*
...
politeness_sel <- politeness %>% 
  filter(!is.na(f0mn)) %>% #making sure there is no NA in the new
...
*It is the single level model ms, who by eyeballing the data appears to be closer to the line. This makes sense, since we are here modeling mean values, meaning that the datapoints have been pulled closer to the great mean.* 
...
```
*In a perfect world, this model would have made the datapoints fit the line better. This is at first sight hard to see, but the fit seem a bit matching with the multilevel model. However both of them appears to be right scewed, which mean that the residuals of the models are not perfectly distributed normally. This is however the least important of the assumptions, (normality of residuals).*
...
```
*Since stat_smooth and I weren't friends, the black dots (not lines sorry) are the plottet fitted values for each of the subjects for each of the scenarios.*
...
model5 <- lmer(f0mn ~ gender.f + attitude.f + (1|scenario.f)+(1|subject), data = politeness)
```

    ii. make a separate model that besides the main effects of _attitude_ and _gender_ also include their interaction
    
```{r}
model6 <- lmer(f0mn ~ gender.f + attitude.f + gender.f*attitude.f + (1|scenario.f)+(1|subject), data = politeness)
summary(model6)
```
...
*GenderfM is then when we go from female to male on the x axe,  we see how the average pitch decrease with 118 hz.*
*attitudef.pol, when we go from informal to polite does the does the average (of both females and males) pitch decrease with 17 hz. This is why need the interaction, so we can consider more than just the average*

*genderM:attitudepol: generelt falder pitch med 15 hz fra informal til polite, men for mænd falder den 5 hz mindre end for kvinder = (-15.646 - 5,781) = -10 agtig for mænd, mens for kvinder falder den -15.646*

*genderM:attitudepol: this is the interaction between gender and attitude, and it indicates that it decreases 5,5hz less for men than women. This means that the change in pitch for men are on -17.192+5.54 = -12 ca, whereas the womens changes with -17.192 hz.*
*Summarizingly, both men and women decrease their pitch when going from informal polite, but the male pitch does not decrease as much as for the women. *
...
var(residuals(model4)) #accumulated residual variance 
var(residuals(model5))
var(residuals(model6))

#R^2 explains how much of the data is explained by the model (here in %)
### using R^2 since it is easier to interpret 
rsq4 <- r.squaredGLMM(model4)*100 
rsq5 <- r.squaredGLMM(model5)*100
rsq6 <- r.squaredGLMM(model6)*100
...
*model 6 with interaction - since it *
