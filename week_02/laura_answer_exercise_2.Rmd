...
*The dataset is a result of a study, which investigated the properties of formal and informal speech register. To do so different variables were measured, to enlighten what might characterize the register. The variables are what we see in the dataset: the frequency of the sentence uttered i Hz(f0mn), different sentences they were to utter (scenarios), gender (gender), duration of the utterance (duration), and the amount of loud hissing breath intake (hiss_count). The two last is the ID of the participant (ID) and the attitude (attitude) they were asked to use when uttering the sentence, this being either formal(pol) or informal(inf).*
...
summary(F1_model2)
```
*When the scenario is an interger the model it considers the scenarios as numbers who follow a the usual number scale - it hereby beliefs that *
*When factor *
